...
/**
 * 
 * 生成的ID用于表示特定资源。如：PAY_AAGSP3DKRCX4AAJTYV3J6， REFUND_AWSEP3DKRCX4AAJT3456
 * 
 * @description 用于生成无意义并且无序的ID
 * @author Roger
 * @date 20170314
 *
 */
public class RestIdGenerator {

	/**
	 * 把trade_no和mer_date变成一个ID，该ID可以通过idDecode()方法反向解析为trade_no和mer_date
	 * 算法： tradeNo+merDate -> base32 encode ->
	 * 加入二位校验码（CRC16） -> 加前缀返回
	 * 
	 */
	

	public static String generate(String tradeNo, String merDate, String prefix) {
		if (merDate.length()!=8) {
			throw new RuntimeException(" merDate is error , ["+merDate+"] is not yyyyMMdd");
		}
		
		byte[] trandNoBytes=null ;
		byte[] merDateBytes=null ;
		try {
			trandNoBytes = tradeNo.getBytes("UTF-8");
			merDateBytes = merDate.getBytes("UTF-8");
		...
		// 最后一位为校验位
		byte[] result = new byte[trandNoBytes.length + merDateBytes.length+1];
		System.arraycopy(trandNoBytes, 0, result, 0, trandNoBytes.length);
		System.arraycopy(merDateBytes, 0, result, trandNoBytes.length, merDateBytes.length);
		// 添加校验位
		result[result.length-1] = getCheckSum(Arrays.copyOfRange(result, 0, result.length-1));

		String base32EncodeStr = Base32.encode(result);
		return prefix + base32EncodeStr;
	}

	/**
	 * 把Paymert_ID或Refund_ID转为trade_no和mer_date.<br>
	 * result[0]: trade_no; result[1]: mer_date; 
	 * @param strId
	 * @param prefix
	 * @return String Array. result[0]: trade_no; result[1]: mer_date; 
	 */
	public static String[] idDecode(String strId, String prefix) {
		byte[] srcBytes = Base32.decode(strId.substring(prefix.length()));

		String tradeNoLong = new String(Arrays.copyOfRange(srcBytes, 0, srcBytes.length-9));
		String merDateInt = new String(Arrays.copyOfRange(srcBytes, srcBytes.length-9, srcBytes.length-1));

		String[] result = { String.valueOf(tradeNoLong), String.valueOf(merDateInt) };
		return result;
	}
	 ...
	 * @return
	 */
	public static boolean isValid(String strId, String prefix){
		try {
			byte[] srcBytes = Base32.decode(strId.substring(prefix.length()));
			byte checksum = getCheckSum(Arrays.copyOfRange(srcBytes, 0, srcBytes.length-1));
			return checksum == srcBytes[srcBytes.length-1];
		} catch (Exception e) {
			return false;
		}
	}
	...
	public static void main(String[] args) {
		String generate = generate("037030710503464320", "20170102", "PAY_");
		System.out.println(generate);
		System.out.println(isValid(generate, "PAY_"));
		String[] parsedRs = idDecode("PAY_GM3TANBSGUYTIMBWGAYDANRTGIZDAMJXGA2DENPZ", "PAY_");
		System.out.println(parsedRs[0]);
		System.out.println(parsedRs[1]);
	}
	...
	private static byte getCheckSum(byte[] data) {
        ...
        return new Integer(res).byteValue();
    }
}
